#include <iostream>
#include <cstring>
#include "Mystring.h"


// No args constructor
Mystring::Mystring()
    : str {nullptr} {
        str = new char[1];
        *str = '\0';
}

// Overloaded constructor
Mystring::Mystring(const char* s)
    : str {nullptr} {
        if (s == nullptr) {
            str = new char[1];
            *str = '\0';
        } else {
            str = new char[strlen(s) + 1];
            strcpy(str, s);
        }
}

// Copy constructor
Mystring::Mystring(const Mystring& source)
    : str {nullptr} {
        str = new char[strlen(source.str) + 1];
        strcpy(str, source.str);
        // std::cout << "Copy constructor called" << std::endl;
}

// Move constructor
Mystring::Mystring(Mystring&& source)
    : str {source.str} {
        source.str = nullptr;
        // std::cout << "Move constructor called" << std::endl;
}

// Destructor
Mystring::~Mystring() {
    delete [] str;
}

// Copy assignment
Mystring& Mystring::operator=(const Mystring& rhs) {
    // std:: << "Using copy assignment" << std::endl;
    if (this == &rhs) {
        return *this;
    }
    delete [] str;
    str = new char[strlen(rhs.str) + 1];
    strcpy(str, rhs.str);
    return *this;
}

// Move assignment
Mystring& Mystring::operator=(Mystring&& rhs) {
    // std:: << "Using move assignment" << std::endl;
    if (this == &rhs) {
        return *this;
    }
    delete [] str;
    str = rhs.str;
    rhs.str = nullptr;
    return *this;
}


// Member methods
void Mystring::display() const {
    std::cout << "String: " << str << ". Length: " << get_length() << std::endl; 
}

int Mystring::get_length() const {
    return std::strlen(str);
}

const char* Mystring::get_str() const {
    return str;
}



// Now the fun part
// as member methods

// Insertion and Extraction methods must be implemented as non-member function
// because they take the user-defined type as the right arg

// << Insertion operator overload
std::ostream& operator<<(std::ostream& os, Mystring& rhs) {
    os << rhs.str;
    return os;
}

// >> Extraction operator ovverload
std::istream& operator>>(std::istream& is, Mystring& rhs) {
    char* buff = new char[1000];
    is >> buff;         // works in std=c++11 but not c++20 for some reason, read the doc
    rhs = Mystring {buff};
    delete [] buff;
    return is;
}        

// -str Minus sign overload, tolower
Mystring Mystring::operator-() const {
    char* buff = new char[std::strlen(str) + 1];
    std::strcpy(buff, str);
    for (size_t i = 0; i < std::strlen(buff); i++) {
        buff[i] = std::tolower(buff[i]);
    }
    Mystring tmp;
    delete [] buff;
    return tmp;
}

// +str Plus sign overload, concatenate
Mystring Mystring::operator+(const Mystring& rhs) const {
    char* buff = new char[std::strlen(str) + std::strlen(rhs.str) + 1];
    std::strcpy(buff, str);
    std::strcat(buff, rhs.str);
    Mystring tmp {buff};
    delete [] buff;
    return tmp;
}  

// == Equal sign overload
bool Mystring::operator==(const Mystring& rhs) const {
    return (std::strcmp(str, rhs.str) == 0);
}     

// != Unequal sign overload
bool Mystring::operator!=(const Mystring& rhs) const {
    return (std::strcmp(str, rhs.str) != 0);
}      

// < Smaller sign overload
bool Mystring::operator<(const Mystring& rhs) const {
    return (std::strcmp(str, rhs.str) < 0);
}     

// > Larger sign ovverload
bool Mystring::operator>(const Mystring& rhs) const {
    return (std::strcmp(str, rhs.str) > 0);
}

// +=str Plus-equal sign overload, concatenate then assign
Mystring& Mystring::operator+=(const Mystring& rhs) {
    *this = *this + rhs;
    return *this;
}

// * Multiply sign overload, repeat for int times
Mystring Mystring::operator*(int times) const {
    char* buff = new char[std::strlen(str) * times + 1];
    for (int i = 0; i < times; i++) {
        std::strcat(buff, str);
    }
    Mystring tmp {buff};
    delete [] buff; 
    return tmp;
}
// or can do like this
// Mystring Mystring::operator*(int times) const {
//     Mystring tmp;
//     for (int i = 0; i < times; i++) {
//         tmp = tmp + *this;
//     }
//     return tmp;
// }


// *= Multiply sign overload, repeat then assign
Mystring& Mystring::operator*=(int n) {
    *this = *this * n;
    return *this;
}     

// ++ Plus-plus sign overload, post-increment, toupper
Mystring Mystring::operator++(int) {
    Mystring tmp {*this};       // make a copy
    operator++();               // call pre-increment - make sure to call PREincrement
    return tmp;
    
}

// ++ Plus-plus sign overload, pre-increment, toupper
Mystring& Mystring::operator++() {
    for (size_t i = 0; i < std::strlen(str); i++) {
        str[i] = std::toupper(str[i]);
    }
    return *this;
}
